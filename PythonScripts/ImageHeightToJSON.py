...
def get_white_pixel_rectangles(image_path):
    ...
    def find_rectangle(x, y):
        ...
        # Expand Diagonally
        while (max_x + 1 < width and max_y + 1 < height and is_white(max_x + 1, max_y + 1) and all(is_white(max_x + 1, k) and is_white(k, max_y) for k in range(min_x, max_x + 1))):
            max_x += 1
            max_y += 1
        
        # Expand to the right
        while max_x + 1 < width and all(is_white(max_x + 1, k) for k in range(min_y, max_y + 1)):
            max_x += 1

        # Expand downwards
        valid = True
        while max_y + 1 < height and all(is_white(k, max_y + 1) for k in range(min_x, max_x + 1)):
            max_y += 1
