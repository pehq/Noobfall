...
def get_white_pixel_rectangles(image_path):
    ...
    def find_rectangle(x, y):
        ...
        # Expand Diagonally
        while (max_x + 1 < width and max_y + 1 < height and is_white(max_x + 1, max_y + 1) and all(is_white(max_x + 1, k) and is_white(k, max_y) and 
        not visited[k][max_x + 1] and not visited[max_y + 1][k] for k in range(min_x, max_x + 1))):
            max_x += 1
            max_y += 1
        
        # Expand to the right
        while max_x + 1 < width and all(is_white(max_x + 1, k) and not visited[k][max_x + 1] for k in range(min_y, max_y + 1)):
            max_x += 1

        # Expand downwards
        valid = True
        while max_y + 1 < height and all(is_white(k, max_y + 1) and not visited[max_y + 1][k] for k in range(min_x, max_x + 1)):
            max_y += 1
        
        # # Expand to the right
        # while max_x + 1 < width and is_white(max_x + 1, y) and not visited[y][max_x + 1]:
        #     max_x += 1
        
        # # Expand downwards
        # valid = True
        # while valid and max_y + 1 < height:
        #     for col in range(min_x, max_x + 1):
        #         if not is_white(col, max_y + 1) or visited[max_y + 1][col]:
        #             valid = False
        #             break
        #     if valid:
        #         max_y += 1
